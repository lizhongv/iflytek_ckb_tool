...
class ExcelHandler:
    ...
    def read_data(
        ...
        logger.info(f"[READ_FILE] Successfully read {len(inputs)} records")
        return inputs
    ...
    def write_results(self, results: List[AnalysisResult], output_path: Optional[str] = None):
        ...
        if output_path is None:
            ...
            try:
                ...
                # output_filename = f"{self.file_path.stem}_analysis_result_{task_id}.xlsx"
                if task_id:
                    output_filename = f"{self.file_path.stem}_analysis_result_{task_id}.xlsx"
                else:
                    output_filename = f"{self.file_path.stem}_analysis_result.xlsx"
            except Exception:
                output_filename = f"{self.file_path.stem}_analysis_result.xlsx"
        ...
        try:
            ...
            logger.info(f"[SAVE_RESULTS] Analysis results saved to: {output_path}")
        except Exception as e:
