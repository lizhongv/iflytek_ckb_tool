        ...
        try:
            ...
            if self.enabled.get("problem_analysis", False):
                ...
                if self.enabled.get("norm_analysis", False):
                    logger.debug(f"  Executing problem normativity analysis (row {row_index + 1})...")
                ...
                if self.enabled.get("set_analysis", False):
                    logger.debug(f"  Executing problem in/out set analysis (row {row_index + 1})...")
                    # Run synchronous analyzer method in thread pool
            ...
            logger.debug(f"  Problem-side analysis completed (row {row_index + 1})")
...
class RecallAnalysisExecutor:
    ...
    async def analyze_single(self, row_index: int, input_data: AnalysisInput) -> AnalysisTaskResult:
        ...
        try:
            if self.enabled.get("recall_analysis", False):
                ...
                if input_data.sources:
                    ...
                    if self.config.chunk_selected and input_data.correct_source:
                        logger.debug(f"  Executing retrieval judgment (by source) (row {row_index + 1}), {len(input_data.sources)} sources...")
                        # Run synchronous analyzer method in thread pool
                    ...
                    # Only execute if chunk_selected is False to avoid duplicate analysis
                        ...
                        logger.debug(f"  Executing retrieval judgment (by answer) (row {row_index + 1}), {len(input_data.sources)} sources...")
                        # Run synchronous analyzer method in thread pool
                        retrieval_result = await asyncio.to_thread(
            ...
            logger.debug(f"  Recall-side analysis completed (row {row_index + 1})")
    ...
    async def analyze_single(self, row_index: int, input_data: AnalysisInput) -> AnalysisTaskResult:
        ...
        try:
            if self.enabled.get("reply_analysis", False) and input_data.model_response:
                logger.debug(f"  Executing response-side analysis (row {row_index + 1})...")
                # Run synchronous analyzer method in thread pool
            ...
            logger.debug(f"  Response-side analysis completed (row {row_index + 1})")
